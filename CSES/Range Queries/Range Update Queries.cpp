...
int query(int id, int ql, int qr)
{
	...
	push(id);
	int m = (l + r) / 2, ans = 0;
	if(ql <= m) ans += query(id * 2, ql, qr);
	if(qr > m) ans += query(id * 2 + 1, ql, qr);
	return ans;
}
void update(int id, int ql, int qr, int val)
{
	int l = seg[id].l, r = seg[id].r;
	if(ql <= l && r <= qr)	seg[id].update(val);
	else
	{
		push(id);
		int m = (l + r) / 2;
		if(ql <= m) update(id * 2, ql, qr, val);
		if(qr > m) update(id * 2 + 1, ql, qr, val);
		pull(id);
	}
}
